...
class GroundingDataset(torch.utils.data.Dataset):
    def __init__(
        ...
        #training data dir, blob/.... in this file
        self.base_image_dir = os.path.join(
            dataset_dir, dataset_mapping[dataset])
        #metadata dir
        ...
        print(
            f"Dataset: {dataset}; Split: {json_data}; # samples: {len(self.json_data)}")
    ...
    def random_crop_metadata(self, img, metadata, scale_range=(0.5, 1.0)):
        ...
        if crop_width > original_width or crop_height > original_height:
            ...
            padded_img = Image.new(
                'RGB', (crop_width, crop_height), (255, 255, 255))
        ...
        for element in metadata['element']:
            ...
            point_abs = [int(point[0] * original_width) + pad_x,
                         int(point[1] * original_height) + pad_y]
    ...
    def get_sample(self, idx):
        ...
        if 'img_url' in item.keys():
            ...
            if self.crop_min != 1 or self.crop_max != 1:
                image_list[0], item = self.random_crop_metadata(
                    image_list[0], item, scale_range=(self.crop_min, self.crop_max))
        else:
            image_path = ""
            image_list = None
        ...
        if len(element_cand) == 1:
            ...
            answer_xy = element['point'] if sample_io in [
                0, 2] else element['bbox']
            if self.xy_int:
            ...
            img_dict = {'type': 'image', 'min_pixels': self.min_pixels,
                        'max_pixels': self.max_pixels}
            source = grounding_to_qwen(element_name, img_dict, sample_io,
                                       self.shuffle_image_token, self.xy_int, self.uniform_prompt)
            prompt = self.processor.tokenizer.apply_chat_template(
                source, tokenize=False, add_generation_prompt=True)
            data_dict_q = self.processor(
                text=prompt, images=image_list, return_tensors="pt", training=not self.inference)
            data_dict_qa, answer = batch_add_answer(
                data_dict_q, answer_xy, self.processor)

        else:
            # multi-turn streaming
            element_name_list = [element['instruction']
                                 for element in element_cand]
            if sample_io in [0, 2]:
            ...
            if self.xy_int:
                answer_xy_list = [[int(x * 1000) for x in answer_xy]
                                  for answer_xy in answer_xy_list]
            else:
                answer_xy_list = [[round(x, 2) for x in answer_xy]
                                  for answer_xy in answer_xy_list]
            ...
            img_dict = {'type': 'image', 'min_pixels': self.min_pixels,
                        'max_pixels': self.max_pixels}
            source = grounding_to_qwen(
                element_name_list[0], img_dict, sample_io, self.shuffle_image_token, self.xy_int, self.uniform_prompt)
            prompt = self.processor.tokenizer.apply_chat_template(
                source, tokenize=False, add_generation_prompt=True)
            data_dict_q = self.processor(
                text=prompt, images=image_list, return_tensors="pt", training=not self.inference)
        ...
        assert data_dict_qa["input_ids"][0].shape[
            0] <= max_seq_len, f"Input seq. surpasses Max. seq len: {data_dict_qa['input_ids'][0].shape[0]} > {max_seq_len}"
...
if __name__ == '__main__':
    ...
    for i in range(len(dataset)):
        ...
        print(i, len(data[0]['input_ids']), data[0]
              ['patch_assign_len'], data[0]['select_mask'].sum())
